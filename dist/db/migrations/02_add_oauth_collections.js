export async function up(db) {
    ...
    await db.collection('oauth_tokens').createIndex({ refreshToken: 1 }, { unique: true, sparse: true });
    await db.collection('oauth_tokens').createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 } // TTL index
    );
    await db.collection('oauth_tokens').createIndex({ userId: 1, clientId: 1 });
    // oauth_codes indices
    await db.collection('oauth_codes').createIndex({ code: 1 }, { unique: true });
    await db.collection('oauth_codes').createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 } // TTL index
    );
    await db.collection('oauth_codes').createIndex({ userId: 1, clientId: 1 });
    // oauth_consents indices
    await db.collection('oauth_consents').createIndex({ userId: 1, clientId: 1 }, { unique: true });
    await db.collection('oauth_consents').createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0, sparse: true } // Optional TTL index
    );
    // audit_logs indices
    await db.collection('audit_logs').createIndex({ timestamp: 1 });
    await db.collection('audit_logs').createIndex({ eventType: 1, timestamp: 1 });
    await db.collection('audit_logs').createIndex({ actorId: 1, timestamp: 1 });
    await db.collection('audit_logs').createIndex({ targetId: 1, timestamp: 1 });
    // Update existing collections
    // Add new indices to users collection
    await db.collection('users').createIndex({ email: 1 }, { sparse: true });
    await db.collection('users').createIndex({ "profile.name": 1 }, { sparse: true });
    await db.collection('users').createIndex({ lastLoginAt: 1 });
    // Add new indices to user_auth_data collection
    await db.collection('user_auth_data').createIndex({ userId: 1, provider: 1 }, { unique: true });
    await db.collection('user_auth_data').createIndex({ provider: 1, providerId: 1 }, { unique: true });
...
//# sourceMappingURL=02_add_oauth_collections.js.map