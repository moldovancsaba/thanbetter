import { OAuthError, validateRedirectUri, verifyPKCE, generateAuthCode, isValidScope, } from './types';
...
export class OAuthService {
    ...
    async createAuthorizationCode(request, userId, identifier) {
    ...
    generateTokens(userId, clientId, scope) {
    ...
    async handleAuthorizationCodeGrant(request) {
        ...
        if (storedCode.codeChallenge && request.codeVerifier) {
            const validPKCE = verifyPKCE(request.codeVerifier, storedCode.codeChallenge, storedCode.codeChallengeMethod || 'plain');
            if (!validPKCE) {
        ...
        const { accessToken, refreshToken, expiresIn } = this.generateTokens(user.id, request.clientId, storedCode.scope);
        return {
            accessToken,
            tokenType: 'Bearer',
            expiresIn,
    ...
    async handleClientCredentialsGrant(request) {
        ...
        const { accessToken, expiresIn } = this.generateTokens(client.id, client.clientId, request.scope);
        return {
            accessToken,
            tokenType: 'Bearer',
            expiresIn,
    ...
    async handlePasswordGrant(request) {
        ...
        const { accessToken, refreshToken, expiresIn } = this.generateTokens(user.id, request.clientId, request.scope);
        return {
            accessToken,
            tokenType: 'Bearer',
            expiresIn,
    ...
    async handleRefreshTokenGrant(request) {
        ...
        const tokenEntry = Array.from(this.tokens.values()).find((t) => t.refreshToken === request.refreshToken);
        ...
        // Generate new tokens
        const { accessToken, refreshToken, expiresIn } = this.generateTokens(tokenEntry.userId, request.clientId, request.scope || tokenEntry.scope);
        return {
            accessToken,
            tokenType: 'Bearer',
            expiresIn,
